...
# source licenses:
# eval_3D_lane.py       The PersFormer Authors        Apache License, Version 2.0
#
...
class LaneEval(object):
    def __init__(self, top_view_region):
        self.dataset_name = 'openlane'
        # self.dataset_dir = args.dataset_dir

        self.x_min = top_view_region[0, 0]
        self.x_max = top_view_region[1, 0]
        self.y_min = top_view_region[2, 1]
        self.y_max = top_view_region[0, 1]
        self.y_samples = np.linspace(self.y_min, self.y_max, num=100, endpoint=False)
        self.dist_th = 1.5
        self.ratio_th = 0.75
        self.close_range = 40

    def bench(self, pred_lanesx,pred_lanesz,  gt_lanex,gt_lanez, gt_visibility=None,pred_category=None, gt_category=None,  P_g2im=None):
        """
        ...
        cnt_gt = len(gt_lanex)
        cnt_pred = len(pred_lanesx)
        pred_lanes=np.zeros((cnt_pred,100,2))
        gt_lanes=np.zeros((cnt_gt,100,2))
        gt_visibility_mat = np.zeros((cnt_gt, 100))
        pred_visibility_mat = np.zeros((cnt_pred, 100))

        # resample gt and pred at y_samples
        for i in range(cnt_gt):
            min_y = np.min(np.array(gt_lanex[i])[:, 1])
            max_y = np.max(np.array(gt_lanex[i])[:, 1])
            x_values, z_values, visibility_vec = resample_laneline_in_y_pred(gt_lanex[i],gt_lanez[i], self.y_samples, out_vis=True)
            # import ipdb
            # #设置断点
            # ipdb.set_trace()
        ...
        for i in range(cnt_pred):
            ...
            min_y = np.min(np.array(pred_lanesx[i])[:, 1])
            max_y = np.max(np.array(pred_lanesx[i])[:, 1])
            x_values, z_values, visibility_vec = resample_laneline_in_y_pred(pred_lanesx[i],pred_lanesz[i], self.y_samples, out_vis=True)
        ...
        for i in range(cnt_gt):
            for j in range(cnt_pred):
                ...
                euclidean_dist[np.isnan(euclidean_dist)]=1000
                x_dist[np.isinf(x_dist)]=1000
                z_dist[np.isinf(z_dist)]=1000
                # import ipdb
                # #设置断点
                # ipdb.set_trace()
    ...
    def bench_final(self, pred_lanes, pred_category, gt_lanes, gt_visibility, gt_category):
        """
            Matching predicted lanes and ground-truth lanes in their IPM projection, ignoring z attributes.
            x error, y_error, and z error are all considered, although the matching does not rely on z
            The input of prediction and ground-truth lanes are in ground coordinate, x-right, y-forward, z-up
    ...
    def bench_new(self, pred_lanes, pred_category, gt_lanes, gt_visibility, gt_category):    
        """
            Matching predicted lanes and ground-truth lanes in their IPM projection, ignoring z attributes.
            x error, y_error, and z error are all considered, although the matching does not rely on z
            The input of prediction and ground-truth lanes are in ground coordinate, x-right, y-forward, z-up
    ...
    def bench_ori(self, pred_lanes, pred_category, gt_lanes, gt_visibility, gt_category, P_g2im=None):
        """
            Matching predicted lanes and ground-truth lanes in their IPM projection, ignoring z attributes.
            x error, y_error, and z error are all considered, although the matching does not rely on z
            The input of prediction and ground-truth lanes are in ground coordinate, x-right, y-forward, z-up
    ...
    def bench_gen(self, pred_lanes,  gt_lanes, P_g2im=None):
        """
            Matching predicted lanes and ground-truth lanes in their IPM projection, ignoring z attributes.
            x error, y_error, and z error are all considered, although the matching does not rely on z
            The input of prediction and ground-truth lanes are in ground coordinate, x-right, y-forward, z-up
        ...
        gt_visibility_mat = np.zeros((cnt_gt, 100))
        pred_visibility_mat = np.zeros((cnt_pred, 100))
        gt_laness=np.zeros((cnt_gt,100,2))
        pred_laness=np.zeros((cnt_pred,100,2))
        # resample gt and pred at y_samples
        for i in range(cnt_gt):
            min_y = np.min(np.array(gt_lanes[i])[:, 1])
            max_y = np.max(np.array(gt_lanes[i])[:, 1])
            x_values, z_values, visibility_vec = resample_laneline_in_y(np.array(gt_lanes[i]), self.y_samples, out_vis=True)
            gt_laness[i] = np.vstack([x_values, z_values]).T
            gt_visibility_mat[i, :] = np.logical_and(x_values >= self.x_min, np.logical_and(x_values <= self.x_max,
                                                     np.logical_and(self.y_samples >= min_y, self.y_samples <= max_y)))
            gt_visibility_mat[i, :] = np.logical_and(gt_visibility_mat[i, :], visibility_vec)
        ...
        for i in range(cnt_pred):
            ...
            max_y = np.max(np.array(pred_lanes[i])[:, 1])
            x_values, z_values, visibility_vec = resample_laneline_in_y(np.array(pred_lanes[i]), self.y_samples, out_vis=True)
            pred_laness[i] = np.vstack([x_values, z_values]).T
            pred_visibility_mat[i, :] = np.logical_and(x_values >= self.x_min, np.logical_and(x_values <= self.x_max,
                                                       np.logical_and(self.y_samples >= min_y, self.y_samples <= max_y)))
            pred_visibility_mat[i, :] = np.logical_and(pred_visibility_mat[i, :], visibility_vec)
            # pred_visibility_mat[i, :] = np.logical_and(x_values >= self.x_min, x_values <= self.x_max)
        ...
        for i in range(cnt_gt):
            for j in range(cnt_pred):
                x_dist = np.abs(gt_laness[i][:, 0] - pred_laness[j][:, 0])
                z_dist = np.abs(gt_laness[i][:, 1] - pred_laness[j][:, 1])
                euclidean_dist = np.sqrt(x_dist ** 2 + z_dist ** 2)
    ...
    def bench_one_submit(self, pred_lines_sub, gt_lines_sub, prob_th=0.5, vis=False):
        ...
        for i, pred in enumerate(json_pred):
            ...
            # evaluate lanelines
            gt_lanes = gt['laneLines']
            gt_visibility = gt['laneLines_visibility']
            gt_category = [] # if no category label
        ...
        R_lane = np.sum(laneline_stats[:, 0]) / (np.sum(laneline_stats[:, 3]) + 1e-6)
        P_lane = np.sum(laneline_stats[:, 1]) / (np.sum(laneline_stats[:, 4]) + 1e-6)
        C_lane = np.sum(laneline_stats[:, 2]) / (np.sum(laneline_stats[:, 5]) + 1e-6)
